...
func main() {
	...
	w, err := seekable.NewWriter(output, enc, seekable.WithLogger(seekableLogger.WithGroup("writer")))
	if err != nil {
	...
	if verifyFlag {
		...
		reader, err := seekable.NewReader(verify, dec, seekable.WithLogger(seekableLogger.WithGroup("reader")))
		if err != nil {
